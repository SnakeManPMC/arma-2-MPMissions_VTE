...
{
	_x setPosASL [(getPosASL downed_helo select 0) + 50, (getPosASL downed_helo select 1) + 50];
	sleep 0.01;
} forEach units sfteam;

// move NVA alert forces into the helo
{
	_x setPosASL [(getPosASL downed_helo select 0) - 250, (getPosASL downed_helo select 1) - 250];
	sleep 0.01;
